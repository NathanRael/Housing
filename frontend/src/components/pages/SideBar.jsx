...
const SideBar = ({className}) => {
  ...
  return (
    <nav className={`fixed z-40 bg-white w-[268px] h-full flex flex-col items-start justify-between gap-16 px-6 py-6`}>
      <div className="w-full flex flex-col items-start justify-start gap-16 ">
        ...
        <ul className="w-full list-none space-y-6">
          {NAVLINKS.map((item) => (
            <NavLink
              item={item}
              active={item.link.slice(1).toLocaleLowerCase() === pathname.slice(1)}
      ...
      <li  className="w-full text-black cursor-pointer flex items-center justify-start gap-3 text-base font-RobotoMd px-6 py-3">
